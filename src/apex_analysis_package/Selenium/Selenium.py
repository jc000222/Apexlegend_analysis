...
class Account_progress:
    ...
    def use_selenium(self):
        ...
        # lv
        input_lv = driver.find_element(
            By.ID, "account_level-9999-AccountLevel"
        )  # Replace with the actual ID of the input field
        input_lv.clear()
        input_lv.send_keys(self.lv)
        time.sleep(1)
        # Purchased_packs
        input_pp = driver.find_element(
            By.ID, "purchased-9999-Purchasedpacks"
        )  # Replace with the actual ID of the input field
        input_pp.clear()
        input_pp.send_keys(self.pp)
        time.sleep(1)
        # seasons
        for i in tqdm(range(int(self.ps), 19)):
            input_seasons = driver.find_element(
                By.ID, f"battlepass-{i}-Season{i}Battlepass"
            )  # Replace with the actual ID of the input field
            input_seasons.clear()
            input_seasons.send_keys(110)  # Replace with the max level
            time.sleep(1)
            checkbox = driver.find_element(
                By.ID, f"battlepass-{i}-Season{i}Battlepass_bp"
            )
